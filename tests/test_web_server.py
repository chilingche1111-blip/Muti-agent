...
class WebApplicationTests(unittest.TestCase):
    def test_static_ui_assets_exist(self) -> None:
        ...
        self.assertIn('id="agent-monitor"', index_html)
        self.assertIn('id="agent-monitor-toggle"', index_html)
        self.assertIn('id="agent-monitor-close"', index_html)
        self.assertLess(index_html.index('id="agent-monitor"'), index_html.index('id="result-panel"'))
        app_js = (WEB_ROOT / "app.js").read_text(encoding="utf-8")
        styles = (WEB_ROOT / "styles.css").read_text(encoding="utf-8")
        self.assertIn("CONVERSATION_STORAGE_KEY", app_js)
        self.assertIn("createAgentVisualization", app_js)
        self.assertIn("createSourceDisclosure", app_js)
        self.assertIn("article.append(evidence, answer)", app_js)
        self.assertIn("turn-source-disclosure", styles)
        self.assertIn("renderAgentMonitor", app_js)
        self.assertIn("setAgentMonitorOpen", app_js)
        self.assertIn("agent-allocation-panel", styles)
