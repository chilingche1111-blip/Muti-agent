...
flowchart TD
    ...
    R -->|"通用问答"| G["原始 LLM\n有界对话记忆"]
    R -->|"原文/指定页码"| P["确定性文档读取器\n0 次 LLM 调用"]
    R -->|"文档研究"| L["LangGraph 确定性外循环"]
...
| 通道 | 适用问题 | 是否使用 Agent Loop | 是否要求文档引用 |
|---|---|---:|---:|
| 通用问答 | 写作、解释、翻译、代码、普通聊天 | 否 | 否 |
| 确定性原文读取 | 输出全文、查看第 16–20 页 | 否 | 直接返回解析原文 |
| 长文档研究 | 总结、事实、风险、比较、跨位置问题 | 是 | 是 |
